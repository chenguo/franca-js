...
It is often useful to query a database for counts of values for a specific field. For example, let's consider a database of fruit:
...
It is also possible to sort by facet value and in ascending order via the ```facet.sort``` subkey. When specifying sorting by value, ascending is the default.
...
-1 | count | descending
1 | count | ascending
```{"count": 1}``` | count | ascending
```{"count": -1}``` | count | descending
